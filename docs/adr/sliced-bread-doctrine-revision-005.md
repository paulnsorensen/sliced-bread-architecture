...
- `skills/sliced-bread-audit/sliced-bread-audit.js` carries that positional growth guard and the framework-native entrypoint, composition-root, event-publisher, and egress guidance; `scripts/check-doctrine-sync.mjs` checks the growth-guards block verbatim across its declared consumers.
...
- `scripts/check-doctrine-sync.mjs` — declared-consumer parity check for the growth-guards block.
