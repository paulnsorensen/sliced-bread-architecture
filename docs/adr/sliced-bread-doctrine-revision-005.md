...
- `skills/sliced-bread-audit/sliced-bread-audit.js` carries that positional growth guard and the framework-native entrypoint, composition-root, event-publisher, and egress guidance; `scripts/check-contracts.mjs` checks the `doctrine:growth-guards` block verbatim across its declared consumers, including `skills/sliced-bread-depth/SKILL.md`.
...
- `scripts/check-contracts.mjs` — declared-consumer parity check for the growth-guards block.
